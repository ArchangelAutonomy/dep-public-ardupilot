...
void GCS_MAVLINK::handle_odometry(const mavlink_message_t &msg)
{
    ...
    if (!isnan(m.pose_covariance[0])) {
        posErr = sqrtf((m.pose_covariance[0]+m.pose_covariance[6]+m.pose_covariance[11])/3);
        angErr = sqrtf((m.pose_covariance[15]+m.pose_covariance[18]+m.pose_covariance[20])/3);
    }
                                                              ...
                                                              const uint16_t payload_size)
{
    ...
    if (!isnan(covariance[0])) {
        ...
        angErr = sqrtf((covariance[15]+covariance[18]+covariance[20])/3);
    }
